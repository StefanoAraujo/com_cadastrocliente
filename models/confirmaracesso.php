...
class CadastroClienteModelConfirmarAcesso extends JModelForm
{
			...
			$query =   "SELECT *
		   	...
		   	if ($soapcliente->sucesso==true) {
		   		if( strtotime(date('Y-m-d H:i:s')) <= strtotime("+3 day", strtotime($soapcliente->conteudo[0]->dataCadastro)))
		   		{
		   			...
		   			SELF::redirecionar("","confirmaracesso");
		   		}else {
	...
	public function incluirsenha()
	{
		try {
			...
			$cadastro = CadastroClienteHelperUtilitarios::pegarSessao("cadastroNovoCliente");
			$entrada = JFactory::getApplication()->input;
			...
			$query =   "UPDATE login SET 
		   	...
		   	$query1 =   "INSERT INTO preCadastroCliente
							           ...
							           ,cpfcnpj)
							     VALUES
							           (".((strlen($cadastro->cpfCnpj)==11)?1:2).",".$cadastro->cpfCnpj.")";
		   	$soapcliente =  CadastroClienteHelperMSSQLService::getConnection($query1); 
		   	if ($soapcliente->sucesso==true) {
		   		CadastroClienteHelperUtilitarios::deletarSessao("cadastroNovoCliente");
